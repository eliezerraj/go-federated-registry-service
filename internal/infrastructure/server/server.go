...
import(
	...
	app_http_routers "github.com/go-federated-registry/internal/infrastructure/adapter/http"
...
// Route constants
const (
	version          = "/v1/federated-registry"
	routeHealth      = "/health"
	routeLive        = "/live"
	routeHeader      = "/header"
	routeContext     = "/context"
	routeInfo        = "/info"
	routeQueryVector = "/query/vector"
	routeExecuteVectorSimilarity = "/execute/similarity"
)
...
func (h *HttpAppServer) setupRoutes(appHttpRouters app_http_routers.HttpRouters) *mux.Router {
	...
	getNoMetrics.HandleFunc(version+routeHealth, appHttpRouters.Health)
	getNoMetrics.HandleFunc(version+routeLive, appHttpRouters.Live)
	getNoMetrics.HandleFunc(version+routeHeader, appHttpRouters.Header)
	getNoMetrics.HandleFunc(version+routeContext, appHttpRouters.Context)
	getNoMetrics.HandleFunc(version+routeInfo, appHttpRouters.Info)
	...
	post.HandleFunc(version+routeQueryVector, h.withMetrics(appMiddleWare.MiddleWareErrorHandler(appHttpRouters.ConvertQueryToVector)))
	post.HandleFunc(version+routeExecuteVectorSimilarity, h.withMetrics(appMiddleWare.MiddleWareErrorHandler(appHttpRouters.ExecuteVectorSimilarity)))
